...
class Solution
{
    public void print_matrix(int[][] matrix, int size)
    {
	...
	while(iter.hasNext())
	{
	    ...
	    if(str.equals(start)) { start_idx = list.size()-1; }
	    if(str.equals(end)) { end_idx = list.size()-1; }
	}
	...
	for(int i = 0; i < size; i++)
	{
		...
		{
		    int diff = get_diff(list.get(i), list.get(j));
	...
	// try to refresh the row of start string
	...
	while(updated)
	{
	    ...
	    int[] start_row = matrix[start_idx];
		...
		int[] nbr_row = matrix[i];
		
		for(int j = 0; j < size; j++)
		{
		    if(nbr_row[j] < 0) { continue; }
		    else
		    {
			if(start_row[j] < 0 || (nbr_row[j] + start_row[i])< start_row[j])
			{
			    start_row[j] = nbr_row[j] + start_row[i];
			    updated = true;
			}
		    }
		}
	...
	// on error, return 0, -1 + 1 = 0
	// on success, return num_of_move + 1
	return matrix[start_idx][end_idx] + 1;
    }
